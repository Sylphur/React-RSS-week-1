...
test('Ensure that the card component renders the relevant card data;', () => {
  ...
  for (const { name } of items) {
    expect(screen.getByText<HTMLParagraphElement>(`Name: ${name}`)).toBeInTheDocument();
  }
})
...
test('Validate that clicking on a card opens a detailed card component', async () => {
  ...
  expect(link).toHaveAttribute('href', generateLink(paginationData.currPage, paginationData.currPageSize, mockedResponse[0].id));
})